...
@command(pattern="^.magnet", outgoing=True)
...
@command(pattern="^.tor", outgoing=True)
...
@command(pattern=r"^.turl", outgoing=True)
async def magnet_download(event):
	if event.fwd_from:
		return
	var = event.text[5:]
...
@command(pattern="^.ariaRM", outgoing=True)
...
@command(pattern="^.show", outgoing=True)
